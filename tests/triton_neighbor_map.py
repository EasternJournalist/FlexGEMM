...
from flex_gemm.kernels.triton.neighbor_map import (
    ...
    get_conv_output_coords_kernel_size_dilation_torch,
...
def test_build_neighbor_map_from_kernel_size_dilation_matches_reference(
    ...
    if _HAS_CUDA_EXT:
        ...
        triton4_nm = build_neighbor_map_from_kernel_size_dilation_triton(
...
def test_build_backward_neighbor_map_correctness() -> None:
    ...
    # Triton
    bwd_triton = transpose_neighbor_map_triton(fwd_neighbor_map, n_input_coords)
...
def test_transpose_neighbor_map_speed_benchmark() -> None:
    ...
    triton_ms = _time_cuda_ms(
        lambda: transpose_neighbor_map_triton(fwd_neighbor_map, n_coords),
        warmup=20,
        iters=100,
    )
...
# ========================================================================================
# ============= get_conv_output_coords_kernel_size_dilation (strided spconv) ==================
# ========================================================================================
...
def test_get_conv_output_coords_kernel_size_dilation_matches_torch(
    kernel_size, stride, dilation, offset, boundary, n_points, coord_range, dtype, tag
) -> None:
    device = torch.device("cuda")
    ...
    ref_coords, _ref_fwd, ref_bwd = get_conv_output_coords_kernel_size_dilation_torch(
        coords, kernel_size, stride=stride, offset=offset, dilation=dilation, boundary=ref_boundary
    )
    tri_coords, tri_fwd, tri_bwd = get_conv_output_coords_kernel_size_dilation_triton(
    ...
    if cuda_compatible:
        ...
        tri_coords_fix, tri_fwd_fix, tri_bwd_fix = get_conv_output_coords_kernel_size_dilation_triton(
...
def test_get_conv_output_coords_kernel_size_dilation_speed_benchmark(
    N, D, kernel_size, stride, dilation, offset, coord_range
) -> None:
    device = torch.device("cuda")
    ...
    torch_ms = _time_cuda_ms(
        lambda: get_conv_output_coords_kernel_size_dilation_torch(
    ...
    triton_ms = _time_cuda_ms(
        lambda: get_conv_output_coords_kernel_size_dilation_triton(
            coords, kernel_size, stride=stride, dilation=dilation, offset=offset, boundary=boundary,
    ...
    M = get_conv_output_coords_kernel_size_dilation_triton(
        coords, kernel_size, stride=stride, dilation=dilation, offset=offset, boundary=boundary,
        return_neighbor_maps=False,
    )[0].shape[0]
    ...
    print(
        f"\n[get_conv_output_coords benchmark] N={n_unique} M={M} D={D} "
...
# ========================================================================================
# ============= get_conv_output_coords_kernel_delta (arbitrary kernel offsets) ================
# ========================================================================================
...
def test_get_conv_output_coords_kernel_delta_matches_torch(
    kernel_size, stride, dilation, offset, boundary, n_points, coord_range, dtype, tag
) -> None:
    device = torch.device("cuda")
    D = len(boundary) if boundary is not None else len(kernel_size)
    ...
    ref_boundary = boundary if boundary is not None else ((-32768, 32767),) * D
    ref_coords, _ref_fwd, ref_bwd = get_conv_output_coords_kernel_delta_torch(
        coords, delta, stride=stride, offset=offset, boundary=ref_boundary,
    )
    tri_coords, tri_fwd, tri_bwd = get_conv_output_coords_kernel_delta_triton(
...
def test_get_conv_output_coords_kernel_delta_speed_benchmark(
    N, D, kernel_size, stride, dilation, offset, coord_range
) -> None:
    device = torch.device("cuda")
    coords = torch.randint(0, coord_range, (N, D), dtype=torch.int16, device=device)
    ...
    torch_ms = _time_cuda_ms(
        lambda: get_conv_output_coords_kernel_delta_torch(
            coords, delta, stride=stride, offset=offset, boundary=boundary,
        ),
        warmup=3,
        iters=10,
    ...
    triton_ms = _time_cuda_ms(
        lambda: get_conv_output_coords_kernel_delta_triton(
            coords, delta, stride=stride, offset=offset, boundary=boundary,
        ),
        warmup=3,
        iters=10,
    )
    M = get_conv_output_coords_kernel_delta_triton(
        coords, delta, stride=stride, offset=offset, boundary=boundary,
    )[0].shape[0]

    print(
        f"\n[get_conv_output_coords_delta benchmark] N={n_unique} M={M} D={D} "
        f"k={kernel_size} s={stride} d={dilation} o={offset}: "
        f"torch={torch_ms:.3f} ms, triton={triton_ms:.3f} ms, "
        f"speedup={torch_ms / triton_ms:.2f}x"
    )
